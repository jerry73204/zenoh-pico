...
z_result_t _z_open_link(_z_link_t *zl, const _z_string_t *locator, const _z_config_t *session_cfg) {
    ...
    if (ret == _Z_RES_OK) {
        ...
        if (_z_endpoint_tcp_valid(&ep) == _Z_RES_OK) {
            ...
            // that no bus has. That is the wall RFC-0080 hit by making every
...
const _z_sys_net_socket_t *_z_link_get_socket(const _z_link_t *link) {
    switch (link->_type) {
        ...
        case _Z_LINK_TYPE_CAN:
            ...
            // callbacks. Same contract as IVC below: NULL means "no FD to wait
            // on".
            return NULL;
#endif
#if Z_FEATURE_LINK_IVC == 1
        ...
        case _Z_LINK_TYPE_ISOTP:
            // Unlike CAN and IVC above, ISO-TP DOES have a real descriptor: the
